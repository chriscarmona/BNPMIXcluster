...
log_f_post_a <- function( a,
  ...
  log_post <- 0
  if(r>1) {
    log_post <- log_post + sum( log(b+a*(1:(r-1)) ) )
  }
  log_post <- log_post + sum( lgamma(x=mu_star_n_r-a)-lgamma(x=1-a) )
  log_post <- log_post + log_prior_f_a(a=a,alpha=alpha,d_0_a=d_0_a,d_1_a=d_1_a)
