...
sampling_Omega_ij <- function( n_sim_mh=1,
                            Omega_ini,
                            i,
                            j,
                            delta=4,
  ...
  while( (length(omega_ij_chain)-1 < (n_sim_mh+n_burn)) & (it_t_i<max_it_time) ) {
  ...
  # browser()
  if(accept_display) {
    return( list( omega_ij_chain = omega_ij_chain[(n_burn+2):length(omega_ij_chain)],
                  accept_indic = accept_indic[(n_burn+2):length(accept_indic)] ) )
  } else {
    return( omega_ij_chain[(n_burn+2):length(omega_ij_chain)] )
  }
}
