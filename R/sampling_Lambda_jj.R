...
sampling_Lambda_jj <- function( n_sim_mh=1,
                               sigma_jj_ini,j,
                               d_0_z,
                               d_1_z,
                               kappa=1,
  ...
  while( (length(sigma_jj_chain)-1 < (n_sim_mh+n_burn)) & (it_t_i<max_it_time) ) {
    if(verbose) {cat(".")}
  ...
  if(accept_display) {
    #browser()
    return( list( sigma_jj = sigma_jj_chain[(n_burn+2):length(sigma_jj_chain)],
                  accept_indic = accept_indic[(n_burn+2):length(accept_indic)] ) )
  } else {
    return( sigma_jj_chain[(n_burn+2):length(sigma_jj_chain)] )
  }
}
